...
<div style="padding: 20px;">
    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 30px;">
        ...
        <div style="background: #f0f9ff; padding: 12px 16px; border-radius: 8px; border-left: 4px solid #16a34a;">
            <div style="font-size: 24px; font-weight: 700; color: #16a34a;">{{ $tickets->total() }}</div>
    ...
    <div style="display: flex; gap: 12px; margin-bottom: 20px; background: white; padding: 16px; border-radius: 10px; border: 1px solid var(--gray-300); flex-wrap: wrap;">
        ...
        <a href="{{ route('admin.support.tickets', array_merge(request()->query(), ['status' => 'open'])) }}" 
            style="padding: 8px 16px; border-radius: 6px; text-decoration: none; font-size: 13px; font-weight: 500; {{ $status === 'open' ? 'background: #16a34a; color: white;' : 'background: var(--bg-1); color: var(--gray-700);' }}">
            Open
        </a>
        <a href="{{ route('admin.support.tickets', array_merge(request()->query(), ['status' => 'in_progress'])) }}" 
            style="padding: 8px 16px; border-radius: 6px; text-decoration: none; font-size: 13px; font-weight: 500; {{ $status === 'in_progress' ? 'background: #16a34a; color: white;' : 'background: var(--bg-1); color: var(--gray-700);' }}">
            In Progress
        </a>
